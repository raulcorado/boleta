...
if (isset($_POST['seguimiento'])) {
     $fechareg = date('Y-m-d');
     if ($_POST['idseg'] > 0) {
          $query = "update seguimiento SET `fecha` = '$_POST[fecha]', `desc` = '$_POST[desc]', `visitaacasa` = $_POST[visitaacasa] WHERE `idseg` = '$_POST[idseg]'";
     } else {
          $query = "insert into seguimiento (`id`,`fecha`,`desc`,`area`,`visitaacasa`) VALUES ('$_SESSION[id]','$_POST[fecha]','$_POST[desc]','$_POST[area]','$_POST[visitaacasa]')";
     }
     ...
     header("location:casoss?id=$_SESSION[id]");
}

if (isset($_POST['cerrarcaso'])) {
     $fechareg = date('Y-m-d');

     $query = "update boletas SET estatusfecha='$_POST[estatusfecha]', idestatus = '$_POST[idestatus]'  WHERE `id` ='$_SESSION[id]'";
...
if (isset($_GET[id])) {
     ...
     $query = "select * from seguimiento s left join boletas b on s.id=b.id left join scactive a on b.sc=`a`.`sc number` where s.id='$_GET[id]' order by s.fecha desc";
     $query = "select * from boletas b left join scactive a on b.sc=a.`sc number` left join seguimiento s on b.id=s.id where b.id='$_GET[id]' order by b.id desc";
               ...
               <div class="col-xs-8">
                    <div class="panel panel-default">
                         ...
                         <div class="panel-body">
                              ...
                              <form role="form" action="casoss" method="post" class="form-horizontal" id="frmpago">
                              ...
                              <table class="table table-condensed table-hover">
                                   ...
                                   <tbody>
                                        <?php
                                        $query = "select * from seguimiento s where s.id='$_GET[id]' order by s.fecha desc";
                                        $result = mysqli_query($link, $query);
                              ...
                              <div class="panel-body">
                                   <table class="table table-condensed table-hover">
                                        <tbody>
                                             ...
                                             <tr>
                                                  <td>EDAD:</td><td><?php echo $rowsc['Age'] ?> años</td>
                                             </tr>
                                             ...
                                             <tr>
                                                  ...
                                                  <td>
                                                       <form role="form" action="casoss" method="post" class="form-horizontal text-danger" id="frmpago">
                                                            <div class="form-group">
